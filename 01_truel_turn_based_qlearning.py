#!/usr/bin/env python3
"""
Turn-based 3-player truel + independent tabular Q-learning self-play.

Notes:
- State = (alive_mask, turn)
- Actions: ("miss", None) or ("shoot", target)
- Terminal reward: winner +1, others -1
- This is the minimal baseline (sparse reward).
...
class TruelEnv:
    ...
    def reset(self, start_turn=0):
        ...
        if not self.alive[self.turn]:
            self.turn = self._next_alive(self.turn)
        return self._state()
    ...
    def step(self, action):
        ...
        if kind == "shoot":
            if target is None or (not self.alive[target]) or target == p:
                raise ValueError(f"Invalid target {target} for state {self._state()}")
...
class QAgent:
    def __init__(self, player_id, alpha=0.2, gamma=0.98, eps_start=1.0, eps_end=0.05, eps_decay=0.99995):
        self.id = player_id
...
def train_truel_qlearning(accuracies=(0.1, 0.8, 0.9), episodes=200_000, seed=42, start_turn=0):
    random.seed(seed)
    np.random.seed(seed)
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            agent.update(state, action, rewards[turn], next_state, next_valid)
            state = next_state
        ...
        if (ep + 1) % 50_000 == 0:
            print(f"Episode {ep+1}/{episodes} | eps={[round(a.eps,3) for a in agents]}")

    return agents

def evaluate_policy(agents, accuracies=(0.1, 0.8, 0.9), games=50_000, seed=7, start_turn=0):
    env = TruelEnv(accuracies, seed=seed)
    wins = [0, 0, 0]
    ...
    for _ in range(games):
        ...
        while not done:
            turn = state[1]
            action = agents[turn].choose_action(env, state)
...
if __name__ == "__main__":
    acc = (0.1, 0.8, 0.9)
    agents = train_truel_qlearning(acc, episodes=200_000, seed=42, start_turn=0)
    wr = evaluate_policy(agents, acc, games=50_000, seed=7, start_turn=0)
    print("Final win rates (A,B,C):", [round(x,4) for x in wr])
